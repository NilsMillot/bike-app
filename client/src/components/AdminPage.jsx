...
import { query, collection, getDocs, where, doc, deleteDoc, setDoc, addDoc } from "firebase/firestore";
...
const AdminPage = () => {
    ...
    }, [currentUserRoles, navigate, isCurrentUserDatasFetched, fetchAllUsersDatas, fetchAllRooms])

    const deleteRoomDoc = useCallback(async(roomName) => {
      try { 
    ...
    const addRoom = useCallback(async() => {
      try {
        ...
        const q = query(collection(db, "rooms"), where("name", "==", newRoom.name));
        const docFetched = await getDocs(q);
    ...
    const modifyRoom = useCallback(async(previousRoomId) => {
      try {
        ...
        const q = query(collection(db, "rooms"), where("name", "==", roomModified.name));
        const docFetched = await getDocs(q);
        if (docFetched.docs.length > 0) {
          alert("Room already exists");
        } else {
  ...
  return (
    <div className="home__container">
        ...
        <tbody>
          {allRooms.map((room) => {
            return (
              <tr key={room.name}>
                <td>
                  {roomModified === room ? (
                    <input type="text" value={roomModified.name} onChange={
                      (e) => setRoomModified(prev => ({...prev, name: e.target.value}))
                    } />
                  ) : (
                    room.name
                  )}
                ...
                <td>
                  {roomModified === room ? (
                    <>
                      <input type="number" value={roomModified.maxUsers} onChange={
                        (e) => setRoomModified(prev => ({...prev, maxUsers: e.target.value}))
                      } /> 
                      <button
                        onClick={() => {
          ...
          <tr>
            <td>
              <input type="text" value={newRoom.name} onChange={
                (e) => setNewRoom(prev => ({...prev, name: e.target.value}))
              } />
            </td>
            <td>
            <input type="number" value={newRoom.maxUsers} onChange={
                (e) => setNewRoom(prev => ({...prev, maxUsers: e.target.value}))
              } />
              <button
                onClick={() => {
                  addRoom()
